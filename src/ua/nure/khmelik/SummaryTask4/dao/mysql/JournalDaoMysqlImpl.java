...
public class JournalDaoMysqlImpl implements JournalDao {
    ...
    @Override
    public ArrayList<Mark> getMarks(Connection conn, int idCourse, int idStudent)
	    throws SQLException {
    ...
    public int deleteCourseControlPoint(Connection conn,
	    int courseControlPointId) throws SQLException {
	try (PreparedStatement pstm = conn.prepareStatement(DELETE_COURSE_CP)) {
	    pstm.setInt(1, courseControlPointId);
	    pstm.executeUpdate();
	} catch (SQLException ex) {
	    // Logger
	    throw ex;
     ...
     */
    @Override
    public int updateMark(Connection conn, Mark m) throws SQLException {
	try (PreparedStatement pstm = conn.prepareStatement(UPDATE_MARK)) {
	    pstm.setInt(1, m.getStudentId());
	    pstm.setInt(2, m.getCourseControlpointId());
	    pstm.setInt(3, m.getValue());
	    pstm.setInt(4, m.getValue());
	    pstm.executeUpdate();
	} catch (SQLException ex) {
	    // Logger
	    throw ex;
	}
	return m.getCourseControlpointId();
    }
